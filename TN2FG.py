...
psi = np.array([[[[2.j, 4.], [3., 0.4]], [[1., 2.], [1.j, 1.]]], [[[1., 1.j], [7., 1 + 2.j]], [[3.j, 0.1], [1., 2.j]]]])
wf = wf.wavefunction()
wf.addwf(psi, 4, 2)
...
plt.figure()
plt.plot(range(len(n2_belief0)), np.real(n2_belief0), 'o')
plt.plot(range(len(n2_belief1)), np.real(n2_belief1), 'o')
plt.plot(range(len(n2_belief1)), np.ones(len(n2_belief0)) * np.real(expectation_z2_wf[0]))
plt.title('$n_2$ (factor graph) vs $S_2$ (mps) measurements')
plt.legend(['$P(n2=+1)$', '$P(n2=-1)$', '$<\psi|\sigma_z(n_2)|\psi>$'])
plt.xlabel('# BP iterations')
plt.ylabel('Probability')
plt.show()

plt.figure()
plt.plot(range(len(n3_belief0)), np.real(n3_belief0), 'o')
plt.plot(range(len(n3_belief1)), np.real(n3_belief1), 'o')
plt.plot(range(len(n3_belief1)), np.ones(len(n3_belief0)) * np.real(expectation_z3_wf[0]))
plt.title('$n_3$ (factor graph) vs $S_3$ (mps) measurements')
plt.legend(['$P(n3=+1)$', '$P(n3=-1)$', '$<\psi|\sigma_z(n_3)|\psi>$'])
plt.xlabel('# BP iterations')
plt.ylabel('Probability')
plt.show()
'''
