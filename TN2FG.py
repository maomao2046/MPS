...
plt.plot(range(len(n0_belief1)), np.ones(len(n0_belief0)) * np.real(expectation_z0_wf[0]))
plt.legend(['$P(n_0=+1)$', '$P(n0=-1)$', '$<\psi|\sigma_z(n_0)|\psi>$'])
plt.xlabel('# BP iterations')
plt.ylabel('Probability')
plt.show()

plt.figure()
plt.plot(range(len(n1_belief0)), np.real(n1_belief0), 'o')
plt.plot(range(len(n1_belief1)), np.real(n1_belief1), 'o')
plt.plot(range(len(n1_belief1)), np.ones(len(n1_belief0)) * np.real(expectation_z1_wf[0]))
plt.legend(['$P(n1=+1)$', '$P(n1=-1)$', '$<\psi|\sigma_z(n_1)|\psi>$'])
plt.xlabel('# BP iterations')
plt.ylabel('Probability')
plt.show()
